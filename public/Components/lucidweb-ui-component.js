...
(function(){
  ...
  AFRAME.registerComponent('gui-cursor', {
      schema: {
          ...
          activeColor: {type: 'string', default: '#ed5b21'},
  ...
  AFRAME.registerComponent('lucidweb-gui-button', {
    ...
    init: function() {
      ...
      this.el.setAttribute("gui-interactable", {});
  ...
  AFRAME.registerComponent('lucidweb-gui-slider', {
    ...
    init: function() {
      ...
      progressBar.setAttribute("gui-interactable", {});
      progressBar.setAttribute('geometry', `primitive: cylinder; height: 7.5; radius: 0.1;`);
      progressBar.setAttribute('material', `shader: flat; opacity: 1;  color: #aaa; side:front;`);
      progressBar.setAttribute("rotation", "0 0 90");
      this.el.appendChild(progressBar);

      this.progressBarColored = document.createElement("a-entity");
      ...
      progressBar.addEventListener("click", this.onClick.bind(this));
      this.progressBarColored.addEventListener("click", this.onClick.bind(this));

      this.progressBarSphere = document.createElement("a-entity");
      this.progressBarSphere.id = "progressBarSphere";
      this.progressBarSphere.setAttribute("geometry", "primitive: sphere; radius: 7.5; radius: 0.2;");
      this.progressBarSphere.setAttribute("material", "shader: flat; opacity: 1;  color: #10418a; side:front;");
      this.el.appendChild(this.progressBarSphere);
    ...
    updateSlider: function(percent){
      ...
      this.progressBarSphere.setAttribute("position", posX + " 0 0");

      posX = -(length / 2) + ((percent * length) / 2);
    ...
    onClick: function(evt){
      ...
      var percentTarget = Math.floor((posX / length) * 1000) / 1000;

      window.player.seekPercent(percentTarget);
    }
  });
